...
public class CifraService {
    ...
    public boolean saveCifra(MultipartFile file, String cifraJson, User user) {
        ...
        try {
            ...
            cifras.setUser(user);
            cifras.setUserName(user.getUserName());
    ...
    public boolean deleteCifra(User user, Long cifraId) {
        return cifrasRepository.findCifrasByUserAndId(user, cifraId).map(cifras -> {
            cifrasRepository.delete(cifras);
            return true;
        }).orElse(false);
    }
